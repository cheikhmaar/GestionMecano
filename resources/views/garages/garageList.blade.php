                ...
                <div class="container-fluid">
                    ...
                    <div class="card">
                        <div class="card-header">
                            <a href="{{route('garageAdd')}}" role="button" class="btn btn-primary">Ajouter une nouvelle garage</a>
                        </div>
                        ...
                        <div class="card-body">
                            <table id="example1" class="table table-bordered table-striped">
                                ...
                                @foreach($garages as $garage)
                                    ...
                                    <tr>
                                        <td scope="row"> {{ $garage->codeGarage  }} </td>
                                        <td>
                                            @foreach($professionnelVehicules as $professionnelVehicule)
                                                @if($professionnelVehicule->id==$garage->professionnel_vehicule_id)
                                                    {{ $professionnelVehicule->nomComplet }}
                                                @endif
                                            @endforeach
                                        </td>
                                        <td> {{ $garage->numeroTelephone  }} </td>
                                        ...
                                        <td>
                                            ...
                                            <a href="/garageList/{{$garage->id}}/garageEdit">
                                                <i class="fas fa-edit text-info"></i>
                                            </a>
                                            <form action="/garageList/{{ $garage->id }}"
                                                  method="POST"
                                                  style="display:inline"
                                                  onsubmit="return confirm('Etes vous sure de vouloir supprimer {{$garage->codeGarage}} {{$garage->nomGarage}} {{}}  de la liste des garage')">
                                                @csrf
                                                @method('DELETE')
